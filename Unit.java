...
public abstract class Unit implements Poolable {
    ...
    public void goTo(int argCellX, int argCellY) {
        ...
        if (stats.movePoints >= gc.getGameMap().getMoveCostCell(argCellX, argCellY)
    ...
    public void update(float dt) {
        ...
        if (!isStayStill()) {
            ...
            if (movementTime > movementMaxTime) {
                ...
                cellX = targetX;
                cellY = targetY;
                stats.movePoints -= gc.getGameMap().getMoveCostCell(cellX,cellY);
    ...
    public boolean amIBlocked() {
        return !((gc.isCellEmpty(cellX - 1, cellY) && this.stats.movePoints >= gc.getGameMap().getMoveCostCell(cellX - 1, cellY))
                || (gc.isCellEmpty(cellX + 1, cellY) && this.stats.movePoints >= gc.getGameMap().getMoveCostCell(cellX + 1, cellY))
                || (gc.isCellEmpty(cellX, cellY - 1) && this.stats.movePoints >= gc.getGameMap().getMoveCostCell(cellX, cellY - 1))
                || (gc.isCellEmpty(cellX, cellY + 1) && this.stats.movePoints >= gc.getGameMap().getMoveCostCell(cellX, cellY + 1)));
    }
}
